    ...
    for (NSDictionary *block in blocks) {
        ...
        if (!blockType) {
            if (block[@"tuple"]) {
                ...
                [self.physicsWorld addJoint: [SKPhysicsJointFixed jointWithBodyA: block1.physicsBody
        ...
        if ([blockType isEqualToString:@"PhotoFrameBlock"]) {
            ...
            [_gameNode addChild: [[OldTVNode alloc]
                                  initWithRect:CGRectFromString(block[@"rect"])]];
        }
        else if ([blockType isEqualToString:@"WonkyBlock"]) {
            [_gameNode addChild:
            [self createWonkyBlockFromRect: CGRectFromString(block[@"rect"])]];
        }
    }
}
...
- (void)didBeginContact:(SKPhysicsContact *)contact
{
    ...
    if (collision == (CNPhysicsCategoryHook|CNPhysicsCategoryCat)) {
        ...
        //2
        SKPhysicsJointFixed *hookJoint = [SKPhysicsJointFixed jointWithBodyA: _hookNode.physicsBody bodyB: _catNode.physicsBody anchor: CGPointMake(_hookNode.position.x,
...
- (void)inGameMessage:(NSString*)text
{
    ...
    // 4
    //  [label runAction: [SKAction sequence:@[[SKAction waitForDuration:3.0],
    //                                         [SKAction removeFromParent]]]];
}
...
- (void)win
{
    ...
    // 5
    [_catNode runAction: [SKAction animateWithTextures:
                          @[[SKTexture textureWithImageNamed:@"cat_curlup1"], [SKTexture textureWithImageNamed:@"cat_curlup2"], [SKTexture textureWithImageNamed:@"cat_curlup3"]]
                                          timePerFrame:0.25]];
...
{
    CGFloat angle = CGPointToAngle(CGPointSubtract(_hookBaseNode.position,
                                                   _hookNode.position));
...
- (void)addHookAtPosition:(CGPoint)hookPosition
{
    ...
    SKPhysicsJointFixed *ceilingFix = [SKPhysicsJointFixed
                                       jointWithBodyA:_hookBaseNode.physicsBody bodyB:self.physicsBody
                                       anchor:CGPointZero];
    ...
    [SKPhysicsBody bodyWithCircleOfRadius: _hookNode.size.width/2];
    ...
    SKPhysicsJointSpring *ropeJoint = [SKPhysicsJointSpring jointWithBodyA:_hookBaseNode.physicsBody bodyB:_hookNode.physicsBody anchorA:_hookBaseNode.position
...
-(void)addSeesawAtPosition:(CGPoint)position
{
    ...
    seesaw.physicsBody.collisionBitMask = CNPhysicsCategoryBlock | CNPhysicsCategoryCat;
    seesaw.physicsBody.categoryBitMask = CNPhysicsCategoryBlock | CNPhysicsCategoryCat;
    seesaw.physicsBody.contactTestBitMask = CNPhysicsCategoryBlock | CNPhysicsCategoryCat;
...
- (void)createPhotoFrameWithPosition:(CGPoint)position
{
 ...
 ///   [_gameNode addChild:photoFrame];
    ...
    photoFrame.physicsBody = [SKPhysicsBody bodyWithCircleOfRadius:
